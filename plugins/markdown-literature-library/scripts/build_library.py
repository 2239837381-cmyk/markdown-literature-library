...
def classify(text: str) -> list[str]:
    """Fallback classification for libraries without curated topic metadata."""
    lowered = text.lower()
...
def metadata_topics(metadata: dict[str, str]) -> list[str]:
    """Use explicit Markdown front matter topics before heuristic matching."""
    raw = metadata.get("topics") or metadata.get("topic") or ""
    if not raw:
        return []
    values = [value.strip() for value in re.split(r"[,，;；/]", raw) if value.strip()]
    return [value for value in values if value in TOPICS]
...
def build_documents(source: Path, recursive: bool) -> list[dict[str, Any]]:
    ...
    for path in files:
        ...
        documents.append({
            ...
            "topics": metadata_topics(metadata) or classify(" ".join([title, tags, body])),
...
def build_page(documents: list[dict[str, Any]], title: str) -> str:
    ...
    topic_data = {key: {"label": value["label"], "color": value["color"], "description": "优先采用 Markdown 标注的主题；未标注时仅作待复核的初步归类。"} for key, value in TOPICS.items()}
